...
class ReplicatorNashMTL(WeightMethod):
    def __init__(
        ...
        super(ReplicatorNashMTL, self).__init__(n_tasks=n_tasks, device=device)
        ...
        self.prvs_alpha = (
            ...
            else np.ones(self.n_tasks, dtype=np.float32)
        )

        # Replicator state
        if x_init is None:
            x_init = torch.ones(self.n_tasks, device=device) / self.n_tasks
    ...
    def _build_payoff_matrix(self, scheduler_features=None, payoff_matrix=None):
        ...
        if scheduler_features is not None and self.alpha_payoff is not None:
            if not isinstance(scheduler_features, torch.Tensor):
                scheduler_features = torch.tensor(
                    scheduler_features,
                    dtype=torch.float32,
                    device=self.device,
                )
            else:
                scheduler_features = scheduler_features.to(self.device)

            alpha_payoff = self.alpha_payoff
            if not isinstance(alpha_payoff, torch.Tensor):
                alpha_payoff = torch.tensor(
                    alpha_payoff,
                    dtype=torch.float32,
                    device=self.device,
                )
            else:
                alpha_payoff = alpha_payoff.to(self.device)
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        if (self.step % self.update_weights_every) == 0:
            ...
            GTG = GTG / self.normalization_factor.item()
            nash_weights = self.solve_optimization(GTG.cpu().detach().numpy())
            nash_weights = torch.from_numpy(nash_weights).to(losses.device)
        else:
            self.step += 1
